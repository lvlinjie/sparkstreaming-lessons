...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    val dataDStream: ReceiverInputDStream[String] = ssc.socketTextStream("localhost",8888,StorageLevel.MEMORY_AND_DISK_SER)
    //2. 数据的处理
    //步骤三 对数据进行处理
    val result = dataDStream.flatMap(_.split(","))
      .map((_, 1))
      .reduceByKey(_ + _)
